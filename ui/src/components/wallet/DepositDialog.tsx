...
export function DepositDialog({
  ...
  }, [selectedMarketIndex, walletSpotBalances, spotMarketConfigs]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(
        `Successfully deposited ${amount} ${selectedSpotMarketSymbol}`,
      );
      setAmount("");
      onOpenChange(false);
    } catch (error) {
      handleErrorToast(error, "Failed to deposit. Please try again.");
    } finally {
      setIsLoading(false);
    }
  };
