...
A slide-by-slide deep-dive: every point on every slide explained in plain
English, plus the cross-questions the professor is most likely to ask
and a confident one-paragraph answer for each.

Use this to prepare. The Q&A is written in the same voice as the
read-aloud script in `PRESENTATION_SCRIPT.md`, so if a question gets
asked you can speak the answer almost as it is.

A guiding principle that runs through every section: **be honest about
what is novel and what is standard practice.** The novelty here is not
that a fee transfer can sit beside a trade — Solana builder-fee
infrastructure already does that for swaps. The novelty is doing it for
*perpetuals* (a much more complex transaction), packaging it as a
teaching system with live attack tests + benchmarks + on-chain CPI
visualisation, and proving every claim with an in-browser demo.
...
* **Middlemen can censor it.** A "relayer" is a service that submits
  user-signed transactions to the blockchain on the user's behalf
  (used for gasless flows, latency reasons, etc.). A malicious relayer
  can simply not forward the fee transaction.
